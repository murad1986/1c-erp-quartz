...
```mermaid
graph LR
    A["ERP 1.0<br/>1960–1990-е<br/>Бухгалтерский сейф"]:::gray --> B["ERP 2.0<br/>1990–2010-е<br/>Операционная система"]:::blue
    B --> C["ERP 3.0<br/>2010–н.в.<br/>Аналитическая платформа"]:::blue
    C --> D["ERP 4.0<br/>Будущее<br/>Decision Engine"]:::green

    A --> A1["Функции:<br/>Бухучёт, зарплата,<br/>складской учёт"]
    B --> B1["Функции:<br/>Планирование, MRP,<br/>управление закупками"]
    C --> C1["Функции:<br/>BI, KPI, сценарное<br/>планирование"]
...
```mermaid
graph TD
    QC["Q-com бизнес-процессы"]:::blue --> R1["Регистр остатков товаров<br/>(Складской учёт)"]:::reg
    QC --> R2["Регистр расчётов<br/>с поставщиками<br/>(Закупки)"]:::reg
    QC --> R3["Регистр ТЗР<br/>(Транспортные расходы)"]:::reg
    QC --> R4["Регистр продаж<br/>(Выручка и возвраты)"]:::reg
    QC --> R5["Регистр себестоимости<br/>(Фактические цены)"]:::reg
    QC --> R6["Регистр ДС<br/>(Эквайринг, банк)"]:::reg
    QC --> R7["Регистр бухгалтерии<br/>(Проводки, после закрытия)"]:::reg
    QC --> R8["Регистр финансового<br/>результата<br/>(P&L)"]:::reg

    R1 --> OUT["Управленческие<br/>решения продакта"]:::green
    R2 --> OUT
    R3 --> OUT
    R4 --> OUT
    R5 --> OUT
...
```mermaid
graph TD
    NOW["ERP 2.5<br/>Сегодня"]:::blue --> T1["Тренд 1<br/>AI-Assisted Planning"]:::ai
    NOW --> T2["Тренд 2<br/>Real-Time ERP"]:::rt
    NOW --> T3["Тренд 3<br/>ERP как Data Platform"]:::dp

    T1 --> T1A["Автопланирование<br/>закупок"]
    T1 --> T1B["Предсказание спроса<br/>по SKU"]
    T1 --> T1C["Оптимизация<br/>смен персонала"]

    T2 --> T2A["Непрерывный<br/>расчёт P&L"]
    T2 --> T2B["Мгновенное<br/>закрытие дня"]
    T2 --> T2C["Real-time<br/>себестоимость"]

    T3 --> T3A["Открытые API<br/>для ML-моделей"]
...
```mermaid
graph TD
    A["1С:ERP 2.5<br/>Сегодня"]:::blue --> B["1С:Аналитика<br/>(BI поверх ERP)"]:::purple
    A --> C["1С:ERP 3.0<br/>(Разработка)"]:::gray
    A --> D["Открытый API<br/>(REST, OData)"]:::green

    B --> B1["Дашборды<br/>на основе регистров"]
    B --> B2["Predictive analytics<br/>(в разработке)"]
    B --> B3["Интеграция<br/>с Power BI / Grafana"]

    C --> C1["Perpetual close<br/>(цель)"]
    C --> C2["AI-рекомендации<br/>(в разработке)"]
    C --> C3["Real-time<br/>себестоимость"]

    D --> D1["ML-модели<br/>поверх ERP-данных"]
...
```mermaid
graph TD
    FAIL["Провал<br/>ERP-трансформации"]:::red --> T["Технические<br/>ошибки<br/>(30%)"]:::orange
    FAIL --> O["Организационные<br/>ошибки<br/>(40%)"]:::orange
    FAIL --> C["Культурные<br/>ошибки<br/>(30%)"]:::orange

    T --> T1["Интеграции без данных:<br/>API есть, данных нет"]
    T --> T2["Миграция данных<br/>провалилась"]
    T --> T3["Производительность:<br/>система не тянет объём"]

    O --> O1["ERP без процессов:<br/>автоматизировали хаос"]
    O --> O2["Нет владельца данных:<br/>никто не отвечает за качество"]
    O --> O3["Дедлайны нарушены:<br/>закрытие каждый раз в панике"]

    C --> C1["Данные без доверия:<br/>люди не верят цифрам системы"]
    C --> C2["Обходные пути:<br/>Excel вместо ERP"]
    C --> C3["Сопротивление изменениям:<br/>люди работают по-старому"]
...
```mermaid
graph LR
    A["PM<br/>логистики<br/>2020"]:::gray --> B["PM<br/>логистики<br/>2025"]:::blue --> C["PM<br/>логистики<br/>2030"]:::green

    A --> A1["Пишет ТЗ<br/>Ставит задачи<br/>Делает исследования"]
    B --> B1["Работает с данными ERP<br/>Понимает финансовые регистры<br/>Строит unit-экономику"]
...
```mermaid
graph TD
    W1["Неделя 1<br/>Фундамент ERP:<br/>НСИ, регистры,<br/>направления деятельности"]:::w --> W2["Неделя 2<br/>Закупки:<br/>Поставщики, заказы,<br/>приёмка товара"]:::w
    W2 --> W3["Неделя 3<br/>Продажи:<br/>Каналы, скидки,<br/>лояльность"]:::w
    W3 --> W4["Неделя 4<br/>Финансы базовые:<br/>Счета, ДС, взаиморасчёты"]:::w
    W4 --> W5["Неделя 5<br/>Склад и инвентаризация:<br/>WMS, остатки, учёт"]:::w
    W5 --> W6["Неделя 6<br/>Планирование:<br/>MRP, прогнозы, OTB"]:::w
    W6 --> W7["Неделя 7<br/>Интеграции:<br/>API, маркетплейсы,<br/>курьерские сервисы"]:::w
    W7 --> W8["Неделя 8<br/>Финансовый результат:<br/>ТЗР, эквайринг,<br/>закрытие, P&L"]:::blue
...
```mermaid
graph LR
    L1["Уровень 1<br/>ERP-грамотность<br/>(этот курс)"]:::green --> L2["Уровень 2<br/>ERP Power User<br/>(6-12 месяцев)"]:::blue
    L2 --> L3["Уровень 3<br/>ERP Product Owner<br/>(12-24 месяца)"]:::blue
...
← [[Неделя 8 - День 6 - Построение управленческого P&L|← День 6]] | [Оглавление](/1c-erp/) | [Оглавление курса](/1c-erp/)
