...
graph TD
    subgraph Система A ["Система учёта A (Юрлицо 1)"]
        A1[Справочник<br/>номенклатуры A]
        A2[Регистр<br/>остатков A]
        A3[Поставщики A]
        A4[Финансы A]
        A1 --- A2
        A2 --- A3
    ...
    subgraph Система B ["Система учёта B (Юрлицо 2)"]
        B1[Справочник<br/>номенклатуры B]
        B2[Регистр<br/>остатков B]
        B3[Поставщики B]
        B4[Финансы B]
        B1 --- B2
        B2 --- B3
...
```mermaid
flowchart LR
    A[Поставщик: Данон] --> B[Молоко<br/>Простоквашино<br/>3,2% 1л]
    B --> C[Система A:<br/>Артикул: MILK-PRK-32-1L<br/>Единица: литр<br/>Категория: Молочная продукция]
    B --> D[Система B:<br/>Артикул: 00-00045821<br/>Единица: штука<br/>Категория: Молоко]
    B --> E[Система B:<br/>Артикул: 00-00045821-DUP<br/> Создан оператором<br/>Категория: Молоко и сливки]
    C --> F{Какой артикул<br/>использовать<br/>для единого<br/>заказа?}
    D --> F
    E --> F
...
```mermaid
flowchart TD
    A[Выгрузка всех позиций<br/>из обеих систем] --> B[Автоматическое<br/>сопоставление<br/>по штрихкоду / GTIN]
    B --> C{Совпадение<br/>найдено?}
    C -->|Да| D[Автоматическое<br/>слияние позиций<br/>~60% позиций]
    C -->|Нет| E[Ручная проверка<br/>товарным экспертом]
    E --> F{Это один<br/>товар?}
    F -->|Да| G[Слияние<br/>с переносом истории]
    F -->|Нет| H[Создание новой<br/>единой позиции<br/>или сохранение обеих]
    D --> I[Единый справочник<br/>номенклатуры]
    G --> I
    H --> I
...
graph TD
    subgraph Физическая реальность
        W[Склад Даркстор №5<br/>350 единиц молока<br/>фактически]
    end

    subgraph Система A
        RA[Регистр A:<br/>Даркстор №5<br/>200 единиц молока]
    end

    subgraph Система B
        RB[Регистр B:<br/>Даркстор №5<br/>180 единиц молока]
    end

    W --- RA
    W --- RB
    RA -.->|200 + 180 = 380<br/>НО! Дублирование| Conflict{КОНФЛИКТ}
    RB -.-> Conflict
    Conflict --> Q1[Какой системе верить<br/>при маршрутизации заказа?]
...
graph TD
    subgraph Группа компаний DC
        H[Холдинг<br/> ООО DC Group]
        H --> L1[ООО DC Logistics<br/>Курьерские услуги]
        H --> L2[ООО DC Retail<br/>Даркстторы]
        H --> L3[ООО DC Tech<br/>Разработка]
        H --> L4[ООО DC Regions<br/>Региональные операции]
    end
...
```mermaid
graph TD
    A[Оценка готовности ERP<br/>к масштабированию] --> B[Справочники]
    A --> C[Производительность]
    A --> D[Интеграции]
    A --> E[Аналитика]
    B --> B1{Справочник<br/>номенклатуры<br/>стандартизирован?}
    C --> C1{Система держит<br/>пиковую нагрузку<br/>без деградации?}
    D --> D1{Интеграции<br/>с каналами<br/>масштабируются?}
    E --> E1{Отчёты строятся<br/>в реальном времени<br/>при больших данных?}
    B1 -->|Нет| B2[РИСК: при росте<br/>проблемы умножатся]
    C1 -->|Нет| C2[РИСК: коллапс<br/>при 2x нагрузке]
...
graph TD
    subgraph Каналы продаж
        C1[Собственное<br/>приложение]
        C2[Агрегаторы]
        C3[B2B]
    end

    subgraph Операционный слой
        OMS[OMS<br/>Ордер Менеджмент<br/>Система]
        WMS[WMS<br/>Складская система<br/>Даркстор]
        TMS[TMS<br/>Диспетчеризация<br/>Курьеры]
    end

    subgraph Учётный слой
        ERP[1C:ERP 2.5<br/>Единственный<br/>экземпляр]
    end

    subgraph Аналитика
        DWH[Data Warehouse<br/>Отчётность]
    end
...
```mermaid
graph LR
    A[Продакт-менеджер] --> B[Бизнес-требования<br/>к интеграции]
    A --> C[Приоритизация<br/>этапов миграции]
    A --> D[UAT —<br/>приёмочное тестирование]
    A --> E[Коммуникация<br/>изменений команде]
    B --> F[IT команда<br/>реализует]
    C --> F
...
```mermaid
flowchart TD
    A[Начало миграции] --> B[Шаг 1:<br/>Справочники<br/>номенклатура<br/>контрагенты<br/>сотрудники<br/>склады]
    B --> C[Шаг 2:<br/>Начальные остатки<br/>Документ ввода<br/>остатков]
    C --> D[Шаг 3:<br/>Открытые заказы<br/>и взаиморасчёты]
    D --> E[Шаг 4:<br/>История документов<br/>за нужный период]
    E --> F[Шаг 5:<br/>Проверка и<br/>расхождения]
    F --> G{Расхождения<br/>в пределах<br/>допустимого?}
    G -->|Да| H[Продуктивный<br/>запуск]
...
```mermaid
flowchart TD
    A[Решение о слиянии<br/>М&А событие] --> B[Аудит текущих систем<br/>2-4 недели]
    B --> C[Проектирование<br/>целевой архитектуры<br/>4-8 недель]
    C --> D[Дедупликация<br/>справочников<br/>8-16 недель]
    D --> E[Разработка<br/>интеграционного слоя<br/>12-24 недели]
    E --> F[Тестирование<br/>на копии базы<br/>4-8 недель]
    F --> G{Тест<br/>пройден?}
    G -->|Нет| H[Исправление<br/>ошибок]
    H --> F
    G -->|Да| I[Инвентаризация<br/>День ноль]
    I --> J[Ввод начальных<br/>остатков в ERP]
    J --> K[Продуктивный<br/>запуск]
...
← [[Неделя 7 - День 6 - Практикум по цепочке доставки|← День 6]] | [Оглавление](/1c-erp/) | [[Неделя 8 - День 1 - Себестоимость|Неделя 8. День 1 →]]
