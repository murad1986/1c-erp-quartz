...
```mermaid
flowchart TD
    A["Товар с фрешем<br/>поступает на склад"]:::entry
    B{"Партионный учёт<br/>включён?"}:::check
    C["Создаётся партия<br/>(серия в регистре)"]:::ok
    D{"Учёт дат<br/>годности включён?"}:::check
    E["Срок годности<br/>записывается в партию"]:::ok
    F{"Стратегия отбора<br/>= FEFO?"}:::check
    G["Задание на отбор:<br/>min(Срок годности)"]:::result
    H["Система выбирает<br/>по FIFO или случайно"]:::warn
...
sequenceDiagram
    ...
    Note over RegSeries,RegStock: Два регистра — два разных слоя данных.
RegSeries = «справочник» партий.
RegStock = «движение» товара.
```
...
```mermaid
flowchart LR
    A["Партия<br/>на складе"]:::stock
    B{"Дней до<br/>истечения<br/>≤ 3?"}:::check
    C{"Дней до<br/>истечения<br/>≤ 1?"}:::check
    D["Статус: Норма<br/>Отбирается по FEFO<br/>без ограничений"]:::ok
    E["Статус: Предупреждение<br/>Оператор видит маркер<br/>Автоскидка 30%"]:::warn
    F["Статус: Критично<br/>Запрет отгрузки<br/>Оформить Списание"]:::error
...
```mermaid
flowchart LR
    B1["До FEFO<br/>Списания: 4,2%<br/>50 400 руб/мес<br/>23 жалобы/мес"]:::before
    B2["После FEFO<br/>Списания: 1,1%<br/>13 200 руб/мес<br/>4 жалобы/мес"]:::after
    B3["Экономия<br/>46 800 руб/мес<br/>561 600 руб/год<br/>Окупаемость < 1 мес"]:::result

    B1 -->|"3 месяца<br/>настройки"| B2
    B2 -->|"Итог"| B3
...
← [[Неделя 4 - День 3 - Партионный учет и серийные номера|← День 3]] | [Оглавление](/1c-erp/) | [[Неделя 4 - День 5 - Весогабаритные характеристики|День 5 →]]
