...
flowchart TD
    ...
    B --> C{Знает, где товар?}
    C -->|Да, работает > 2 мес| D[Идёт напрямую<br/>время: ~1 мин 40 сек]
    C -->|Нет, стажёр| E[Обход склада визуально<br/>время: ~4 мин 05 сек]
    D --> F[Берёт товар]
    E --> F
    F --> G{Правильный<br/>товар?}
    G -->|Да — 97,7%| H[Продолжает сборку]
    G -->|Нет — 2,3%| I[Возврат, повторный поиск<br/>+3-4 минуты]
...
```mermaid
graph TD
    A["Склад<br/>(Бутово, ул. Дорожная 3)"] --> B1["Складская территория<br/>(Зона А — Сухой)"]
    A --> B2["Складская территория<br/>(Зона Б — Охлаждение 0-6°C)"]
    A --> B3["Складская территория<br/>(Зона В — Заморозка -18°C)"]
    B1 --> C1["Секция А-01"]
    B1 --> C2["Секция А-02"]
    B1 --> C3["Секция А-03 ... А-22"]
    B2 --> C4["Секция Б-01 ... Б-12"]
    B3 --> C5["Секция В-01 ... В-06"]
    C1 --> D1["Ячейка А-01-П1<br/>(Полка 1)"]
...
```mermaid
flowchart LR
    A[Поставка молока<br/>приходит на склад] --> B{Проверка однородности<br/>сроков годности}
    B -->|Все упаковки —<br/>одна дата| C[Стандартная приёмка<br/>Одна партия в ERP]
    B -->|Разные даты| D[Сортировка поддона<br/>по срокам]
    D --> E[Группа 1:<br/>самый ранний срок]
    D --> F[Группа 2:<br/>средний срок]
    D --> G[Группа 3:<br/>поздний срок]
    E --> H[Партия 1 в ERP<br/>Ячейка Б-04-П1]
    F --> I[Партия 2 в ERP<br/>Ячейка Б-04-П2]
    G --> J[Партия 3 в ERP<br/>Ячейка Б-04-П3]
    C --> K[Сборка по FEFO:<br/>сначала ранняя дата]
    H --> K
    I --> K
    J --> K
...
← [[Неделя 4 - День 6 - Практикум по топологии склада|← День 6]] | [Оглавление](/1c-erp/) | [[Неделя 5 - День 1 - Складские перемещения|День 1. Неделя 5 →]]
